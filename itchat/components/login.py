...
def __recv_check_login(param):
    data = param.data
    paramFunc = param.param
    self = paramFunc["self"]
    uuid = paramFunc["uuid"]
    qrStorage = paramFunc["qrStorage"]
    enableCmdQR = paramFunc["enableCmdQR"]
    picDir = paramFunc["picDir"]
    qrDownFinishCallBack = paramFunc["qrDownFinishCallBack"]
    qrCallback = paramFunc["qrCallback"]
    loginCallback = paramFunc["loginCallback"]
    exitCallback = paramFunc["exitCallback"]

    def GetStatus(data):
        regx = r'window.code=(\d+)'
        dataGroup = re.search(regx, data)
        if dataGroup and dataGroup.group(1) == '200':
            if process_login_info(self, data):
                return '200'
            else:
                return '400'
        elif dataGroup:
            return dataGroup.group(1)
        else:
            return '400'
    ...
    if status == '200':
        ...
        if isLoggedIn is not None:
            ...
            __check_login_async(self,uuid,qrStorage,enableCmdQR, picDir,qrDownFinishCallBack, qrCallback,
                                loginCallback, exitCallback)
    elif status == '408':
        __check_login_async(self,uuid,qrStorage,enableCmdQR, picDir,qrDownFinishCallBack, qrCallback,
                            loginCallback, exitCallback)
        ...
        login(self,enableCmdQR, picDir,qrDownFinishCallBack, qrCallback,
              loginCallback, exitCallback)
    ...
    if isLoggedIn:
        ...
        start_receiving_async(self,exitCallback)
        self.isLogging = False

def __error_check_login(param):
    pass

def __check_login_async(self,uuid,qrStorage,enableCmdQR=False, picDir=None,qrDownFinishCallBack=None, qrCallback=None,
    ...
    paramFunc["self"] = self
    paramFunc["uuid"] = uuid
    paramFunc["qrStorage"] = qrStorage
    paramFunc["enableCmdQR"] = enableCmdQR
    paramFunc["picDir"] = picDir
    paramFunc["qrDownFinishCallBack"] = qrDownFinishCallBack
    paramFunc["qrCallback"] = qrCallback
    paramFunc["loginCallback"] = loginCallback
    paramFunc["exitCallback"] = exitCallback

    HttpTaskManager().CreateTask(url,params,headers,None,"GET",__recv_check_login,__error_check_login,paramFunc)
...
def login(self, enableCmdQR=False, picDir=None,qrDownFinishCallBack=None, qrCallback=None,
        loginCallback=None, exitCallback=None):
    ...
    while self.isLogging:
        ...
        if uuid:
            ...
            if qrDownFinishCallBack:
                qrDownFinishCallBack(status=True)
...
def check_login(self, uuid=None,timeout=None):
    uuid = uuid or self.uuid
    url = '%s/cgi-bin/mmwebwx-bin/login' % config.BASE_URL
    localTime = int(time.time())
    params = 'loginicon=true&uuid=%s&tip=1&r=%s&_=%s' % (
...
def recv_sync_check(param):
    data = param.data
    paramsFunc = param.param
    self = paramsFunc["self"]
    regx = r'window.synccheck={retcode:"(\d+)",selector:"(\d+)"}'
    pm = re.search(regx, data)
    if pm is None or pm.group(1) != '0':
        logger.debug('Unexpected sync check result: %s' % data)
        return None
    i = pm.group(2)
    ...
    if i is None:
        ...
        post_sync_check(self)
    else:
        post_msg_request(self)

def error_sync_check(param):
    paramsFunc = param.param
    self = paramsFunc["self"]
    print "error_sync_check %s %s" % (param.error.code,param.error.msg)
    post_sync_check(self)
...
def post_sync_check(self):
    ...
    headers = { 'User-Agent' : config.USER_AGENT }
    paramsFunc = {}
    paramsFunc["self"] = self

    HttpTaskManager().CreateTask(url,params,headers,None,"GET",recv_sync_check,error_sync_check,paramsFunc,config.TIMEOUT[1])
    '''
    headers = { 'User-Agent' : config.USER_AGENT }
    try:
...
def recv_msg(param):
    ...
    msgList, contactList = get_dict_info(data)
    if msgList:
    ...
    #    PostMsgRequest(self,exitCallback,getReceivingFnOnly)
        return
    pass
def error_recv_msg(param):
    paramsFunc = param.param
    self = paramsFunc["self"]
    print "error_recv_msg %s %s" % (param.error.code,param.error.msg)
    post_msg_request(self)

def post_msg_request(self):
    ...
    paramsFunc = {}
    paramsFunc["self"] = self
    HttpTaskManager().CreateTask(url,None,headers,json.dumps(data),"POST",recv_msg,error_recv_msg,paramsFunc,config.TIMEOUT[1])
    '''
...
def start_receiving_async(self,exitCallback=None, getReceivingFnOnly=False):
